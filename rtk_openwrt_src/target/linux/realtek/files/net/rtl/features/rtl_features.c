		...
		if (rtl_masq_info[i].valid == 1) {
			if (strncmp(rtl_masq_info[i].ifName, name, IFNAMSIZ) == 0) {
				if (rtl_masq_info[i].ipAddr != ipAddr) {
					...
					if (rtl865x_delIp(rtl_masq_info[i].ipAddr) == SUCCESS) {
						rtl865x_nat_init();
					}
					empty = i;
					break;
...
int32 rtl_fn_insert(struct fib_table *tb, struct fib_config *cfg, struct fib_info *fi)
{
	...
	/*2007-12-19*/
#if defined(CONFIG_RTL_HW_NAPT_REFINE_KERNEL_HOOKS)
    if ((tb->tb_id == RT_TABLE_MAIN) && (gHwNatEnabled !=0) && (router_support_hw_nat != 0))
#else
	...
	hlist_for_each_entry_safe(li, tmp, lih, hlist) {
		list_for_each_entry_safe(fa, fa_node, &li->falh, fa_list) {
			...
			if (fi) {
				...
				cfg.fc_dst =  l->key;
				cfg.fc_dst_len = inet_mask_len(li->mask_plen);
...
int rtl_del_masq_info_by_dev_name(const char *name)
{
	...
	{
		entry->valid = 0;
		if (rtl865x_delIp(entry->ipAddr) == SUCCESS) {
			rtl865x_nat_init();
			ret = SUCCESS;
		}
	}
...
static int rtl_sync_ppp_info_to_hw_ppp_table(void)
{
	...
	lookup ppp dev.*/
	if ((ppp_dev=__dev_get_by_name(&init_net, RTL_DRV_PPP_NETIF_NAME)) != NULL) {
...
#ifdef CONFIG_RTL_LAYERED_DRIVER_L3
		rtl865x_detachMasterNetif(RTL_DRV_PPP_NETIF_NAME);
#if 0//def CONFIG_RTL_LAYERED_DRIVER_L2	
		#if defined (CONFIG_RTL_HW_QOS_SUPPORT)
		rtl865x_qosFlushMarkRuleByDev(RTL_DRV_PPP_NETIF_NAME);
		#endif
#endif
		rtl_del_ps_drv_netif_mapping_by_name(RTL_DRV_PPP_NETIF_NAME);	// sync from voip customer for multiple ppp
		rtl865x_delPppbyIfName(RTL_DRV_PPP_NETIF_NAME);
#endif
		ret = SUCCESS;
	}
