...
{
	memset(qosConf,0,RTL_MAX_QOSCONFIG_DEV_NUM*sizeof(qosConf));
	return;
}
		      ...
		      unsigned long count, void *data)
{
    ...
    if (count > 1) {	//call from shell
      	if (buffer && !copy_from_user(&tmpbuf, buffer, 1)) {
			tmpbuf[count] = '\0';

			strptr = tmpbuf;
			tokptr = strsep(&strptr, " ");
			if(!memcpy(tokptr, "opened", 6))
			{	
				tokptr = strsep(&strptr, " ");
				if(tokptr == NULL)
					return -EFAULT;
			...
			else if(!memcpy(tokptr, "fastleave", 9))
			{
				tokptr = strsep(&strptr, " ");
				if(tokptr == NULL)
					return -EFAULT;
	...
	*moreFlag=0;
	...
	if(protoType[0] == 0x3a){
		...
		if(icmpv6h->icmp6_type == ICMPV6_MGM_REPORT){
			...
			#endif
			if(!IN6_IS_ADDR_MULTICAST(icmpv6_McastAddr))
				return -1;
			...
			#ifdef	DBG_ICMPv6
			printk("%s:%d,*gIndex is %d,mldv2report->ngrec is %d\n",__FUNCTION__,__LINE__,*gIndex,mldv2report->mld2r_ngrec);
			#endif
			if(*gIndex >= mldv2report->mld2r_ngrec)
			{
				*moreFlag=0;
				return -1;
			}
			
			for(i=0;i<mldv2report->mld2r_ngrec;i++)
			{
				...
				{
					mldv2grec = (struct mld2_grec *)((unsigned char*)mldv2grec+4+16+mldv2grec->grec_nsrcs*16+(mldv2grec->grec_auxwords)*4);
				}
				...
				if(i==(mldv2report->mld2r_ngrec-1))
				{
					/*last group record*/
					*moreFlag=0;
				}
				...
				*gIndex=*gIndex+1;
				...
				if(((mldv2grec->grec_type== MLD2_CHANGE_TO_INCLUDE) || (mldv2grec->grec_type == MLD2_MODE_IS_INCLUDE))&& (mldv2grec->grec_nsrcs==0))
				{
					return 2; /* leave and delete it */
				}
...
int rtl_IgmpSnooping_Input_Hook(struct net_bridge *br,struct sk_buff *skb,struct sk_buff *skb2)
{
	...
	if (skb)
	{
		if (is_multicast_ether_addr(dest)) {
			if (MULTICAST_MAC(dest)
			&& (eth_hdr(skb)->h_proto == ETH_P_IP) && igmpsnoopenabled)
			{
				iph=(struct iphdr *)skb_network_header(skb);
...
#if defined IGMP_DEBUG
				if(TEST_PACKETS(dest))
				printk("%s:%d, existMfc = %d\n",__FUNCTION__,__LINE__,existMfc);
#endif
...
#if defined IGMP_DEBUG
			if(TEST_PACKETS(dest))
				printk("%s:%d,ret=%d, unkown = %d, fastleave = %d\n",
					__FUNCTION__,__LINE__,ret,multicastFwdInfo.unknownMCast, IGMPPRoxyFastLeave);
#endif
