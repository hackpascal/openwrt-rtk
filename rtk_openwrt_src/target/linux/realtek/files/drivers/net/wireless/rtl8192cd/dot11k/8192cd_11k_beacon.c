...
static unsigned char get_op_class_by_channel(struct rtl8192cd_priv *priv, unsigned char channel)
{
    int i,j;
             ...
             priv->pmib->dot11StationConfigEntry.dot11RMBeaconPassiveMeasurementActivated))
    {
        ...
        if(pstat)
        {
            ...
            if(ret == 0)   /*issue beacon measurement request succeed*/
            {
                ...
                measure_time += pstat->rm.beacon_req.random_interval + 500; //tolerance
...
int rm_collect_bss_info(struct rtl8192cd_priv *priv, struct rx_frinfo *pfrinfo)
{
    ...
    if(request->channel == 255)
        beacon_rep->info.op_class = get_op_class_by_channel(priv, priv->site_survey->ss_channel);
    else
