	...
	)
{
	...
	halComTxbf_beamformSetGtab(pDM_Odm);
	...
	)
{
	...
	PRT_BEAMFORMING_INFO	pBeamInfo = &pDM_Odm->BeamformingInfo;
	PRT_BEAMFORMEE_ENTRY	pBeamformEntry = phydm_Beamforming_GetBFeeEntryByAddr(pDM_Odm, RA, &Idx);
	PADAPTER				BFSourceAdapter = pBeamInfo->SourceAdapter;
	...
	if (MgntGetBuffer(Adapter, &pTcb, &pBuf)) {
		...
		MgntSendPacket(BFSourceAdapter, pTcb, pBuf, pTcb->PacketLength, NORMAL_QUEUE, DataRate);
	} else
		ret = FALSE;
...
/*
// Description: Construct VHT broacast NDPA packet.
