...
)
{
	...
	case PHYDM_PAUSE:
	{
		...
		/* Backup IGI value */
		if (pDM_DigTable->pause_dig_level == BIT(pause_level)) {
	...
	if (pDM_Odm->SupportAbility & ODM_BB_ADAPTIVITY) {
		...
		ODM_RT_TRACE(pDM_Odm,ODM_COMP_DIG, ODM_DBG_LOUD, ("IGI has changed, adjust EDCCA threshold !!\n"));
	}
}
...
)
{
	...
	case PHYDM_PAUSE:
	{
		...
		if (pDM_DigTable->pause_cckpd_level == BIT(pause_level)) {
		...
		if (BIT(pause_level + 1) > pDM_DigTable->pause_cckpd_level) {
			ODM_Write_DIG(pDM_Odm, CCKPDThreshold);
