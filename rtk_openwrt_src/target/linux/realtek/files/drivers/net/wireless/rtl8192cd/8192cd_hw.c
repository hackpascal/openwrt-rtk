...
#define	BAND2G_20M_3T_HT 		14
#define	BAND2G_20M_4T_HT 		15
#define	BAND2G_40M_3T_HT		16
#define	BAND2G_40M_4T_HT		17
#define	BAND5G_20M_3T_HT 		18
#define	BAND5G_20M_4T_HT 		19
#define	BAND5G_40M_3T_HT		20
#define	BAND5G_40M_4T_HT		21
...
void SetTxPowerLevel(struct rtl8192cd_priv *priv)
{
	...
	if (GET_CHIP_VER(priv) == VERSION_8814A) {
		unsigned int path,i;
		priv->pshare->phw->minTxAgcVal = 255;
		for(path=0;path<4;path++) {
			//GDEBUG("CurrentTxAgcCCK[%d][3]=%x\n",path,priv->pshare->phw->CurrentTxAgcCCK[path][3]);
			if(priv->pshare->phw->CurrentTxAgcCCK[path][3]!=0 && 
				priv->pshare->phw->CurrentTxAgcCCK[path][3] < priv->pshare->phw->minTxAgcVal)
				priv->pshare->phw->minTxAgcVal = priv->pshare->phw->CurrentTxAgcCCK[path][3];

			//GDEBUG("CurrentTxAgcOFDM[%d][7]=%x\n",path,priv->pshare->phw->CurrentTxAgcOFDM[path][7]);			
			if(priv->pshare->phw->CurrentTxAgcOFDM[path][7]!=0 &&
				priv->pshare->phw->CurrentTxAgcOFDM[path][7] < priv->pshare->phw->minTxAgcVal)
				priv->pshare->phw->minTxAgcVal = priv->pshare->phw->CurrentTxAgcOFDM[path][7];

			//GDEBUG("CurrentTxAgcMCS[%d][23]=%x\n",path,priv->pshare->phw->CurrentTxAgcMCS[path][23]);
			if(priv->pshare->phw->CurrentTxAgcMCS[path][23]!=0 &&
				priv->pshare->phw->CurrentTxAgcMCS[path][23] < priv->pshare->phw->minTxAgcVal)
				priv->pshare->phw->minTxAgcVal = priv->pshare->phw->CurrentTxAgcMCS[path][23];

			//GDEBUG("CurrentTxAgcVHT[%d][29]=%x\n",path,priv->pshare->phw->CurrentTxAgcVHT[path][23]);
			if(priv->pshare->phw->CurrentTxAgcVHT[path][29]!=0 && 
				priv->pshare->phw->CurrentTxAgcVHT[path][29] < priv->pshare->phw->minTxAgcVal)
				priv->pshare->phw->minTxAgcVal = priv->pshare->phw->CurrentTxAgcVHT[path][29];
		}
		//panic_printk("minTxAgcVal=%x\n", priv->pshare->phw->minTxAgcVal);
		if ((priv->pmib->dot11RFEntry.rfe_type == 3 || priv->pmib->dot11RFEntry.rfe_type == 5) && priv->pshare->phw->minTxAgcVal >= 22) {
			priv->pshare->phw->TXPowerOffset = 3; // -11 dB
		} else if(priv->pshare->phw->minTxAgcVal >= 14){
			priv->pshare->phw->TXPowerOffset = 2; // -7 dB
		} else if(priv->pshare->phw->minTxAgcVal >= 6){
			priv->pshare->phw->TXPowerOffset = 1; // -3 dB
		}
		//panic_printk("minTxAgcVal=%x, TXPowerOffset=%d\n", priv->pshare->phw->minTxAgcVal,priv->pshare->phw->TXPowerOffset);
	}
#endif
