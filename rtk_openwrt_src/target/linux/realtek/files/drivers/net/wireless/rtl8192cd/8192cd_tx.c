...
#endif
	{
#if defined(CONFIG_PUMA_VLAN_8021Q) || defined(CONFIG_USB_HCI) || defined(CONFIG_SDIO_HCI)
		if(*((UINT16*)(skb->data+(ETH_ALEN<<1))) == __constant_htons(ETH_P_8021Q)){
			if (skb_cloned(skb)){
				newskb = skb_copy(skb, GFP_ATOMIC);
				if (newskb == NULL) {
...
#endif
#ifdef CONFIG_IEEE80211W
		!UseSwCrypto(priv, ptxsc_entry->txcfg.pstat, FALSE, 0) ) &&
#else
		!UseSwCrypto(priv, ptxsc_entry->txcfg.pstat, FALSE) ) &&
#endif			
