...
/*Only for MU BFee Entry*/
VOID
halComTxbf_beamformSetGtab(
	IN PVOID			pDM_VOID
	)
{
	PDM_ODM_T	pDM_Odm = (PDM_ODM_T)pDM_VOID;
	...
	if (PlatformIsWorkItemScheduled(&(pTxbfInfo->Txbf_GetGtabWorkItem)) == FALSE)
		PlatformScheduleWorkItem(&(pTxbfInfo->Txbf_GetGtabWorkItem));
#else
	halComTxbf_GetGtabWorkItemCallback(pDM_Odm);
#endif
}
...
/*Only for MU BFee Entry*/
VOID
halComTxbf_GetGtabWorkItemCallback(
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
	IN	PADAPTER		Adapter
#else
	IN PVOID			pDM_VOID
