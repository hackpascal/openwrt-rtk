	...
	)
{
	...
	for (i = 0; i <= 23; i++) {
		beam_ctrl_signal = (BOOLEAN)((beam_pattern&BIT(i)) >> i);
		...
		/*ODM_SetMACReg(pDM_Odm, 0x44 , BIT11|BIT10, ( beam_ctrl_signal<<1 |1));*/
		/*ODM_SetMACReg(pDM_Odm, 0x44 , BIT10, 0); */
		ODM_SetMACReg(pDM_Odm, 0x44 , BIT11|BIT10, ((1<<1) | beam_ctrl_signal));
		ODM_SetMACReg(pDM_Odm, 0x44 , BIT11, 0); 
		ODM_StallExecution(pdm_sat_table->latch_time);/*Wait for latching data */
	}
}
	...
	)
{
	...
	if (dm_value[0] == 1) {
		...
		pdm_sat_table->fix_beam_pattern = dm_value[1];
		
		if (dm_value[1] == 1) {
			pdm_sat_table->beam_pattern = dm_value[2];
			phydm_update_beam_pattern(pDM_Odm, dm_value[2]);
		} 
	...
	)
{
	...
	pdm_sat_table->latch_time = 1; /*mu sec*/
	pdm_sat_table->fix_beam_pattern  = 0;
/*
...
*/
	...
	ODM_SetMACReg(pDM_Odm, 0x44 , BIT19|BIT18, 0x3);	/*enable_output for P_GPIO[1:0]*/
	ODM_SetMACReg(pDM_Odm, 0x44 , BIT11|BIT10, 0);		/*output_value for P_GPIO[1:0]*/
	ODM_SetMACReg(pDM_Odm, 0x40 , BIT1|BIT0, 0);		/*GPIO function*/
	ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("***8821A [phydm_hl_smart_ant_type1_init_8821a]  end\n"));		
}
	...
	)
{
	...
	else if(pDM_Odm->SupportICType == ODM_RTL8821)
	{
		...
		if (pDM_Odm->AntDivType == HL_SW_SMART_ANT_TYPE1) {
			...
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("***8821A[ODM_AntDivInit]  in\n"));
			phydm_hl_smart_ant_type1_init_8821a(pDM_Odm);
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_ANT_DIV, ODM_DBG_LOUD, ("***8821A[ODM_AntDivInit]  end\n"));
		} else {
			/*pDM_Odm->AntDivType = CG_TRX_HW_ANTDIV;*/
			pDM_Odm->AntDivType = S0S1_SW_ANTDIV;
