	...
	)
{
	...
	ODM_MoveMemory(pDM_Odm, pDM_Odm->CurrentAddress, Adapter->CurrentAddress, 6);
	...
	ODM_MoveMemory(pDM_Odm, pDM_Odm->CurrentAddress, Adapter->mac_addr, 6);
...
//
// Return Value: Idex of the first MU sta.
//
	...
	)
{
	...
	if (pEntry != NULL) {	
		...
		if (phydm_actingDetermine(pDM_Odm, PhyDM_ACTING_AS_AP)) {
			...
			u2Byte BSSID = ((pDM_Odm->CurrentAddress[5] & 0xf0) >> 4) ^ (pDM_Odm->CurrentAddress[5] & 0xf);
			/*(dec(A) + dec(B)*32) mod 512*/
			pEntry->P_AID = (AID + BSSID * 32) & 0x1ff;
			pEntry->G_ID = 63;
	...
	)
{
	...
	if (BeamformCap & (BEAMFORMEE_CAP_VHT_MU | BEAMFORMEE_CAP_VHT_SU | BEAMFORMEE_CAP_HT_EXPLICIT)) {
		...
		if (pBeamformerEntry == NULL) {
			pBeamformerEntry = Beamforming_AddBFerEntry(pDM_Odm, pSTA->RA, pSTA->AID, BeamformCap, NumofSoundingDim , &BFerIdx);
			if (pBeamformerEntry == NULL)
	...
	if (BeamformCap & (BEAMFORMER_CAP_VHT_MU | BEAMFORMER_CAP_VHT_SU | BEAMFORMER_CAP_HT_EXPLICIT)) {
		...
		if (pBeamformEntry == NULL) {
			pBeamformEntry = Beamforming_AddBFeeEntry(pDM_Odm, pSTA->RA, pSTA->AID, pSTA->MacID, pSTA->BW, BeamformCap, NumofSoundingDim, CompSteeringNumofBFer, &BFeeIdx);
