...
#if 0
	/* HW limitation. We can't write TXAGC once a byte. */
	for (i = 0; i < 4; i++) {
		if (i != rate_offset)
	...
	)
{
	...
	if ((RxPath == ODM_RF_A) || (RxPath == ODM_RF_B)) {
		...
		/* Disable MRC for CCA */
		ODM_SetBBReg(pDM_Odm, 0xa2e, BIT6, 0x0);

		/* Disable MRC for barker */
		ODM_SetBBReg(pDM_Odm, 0xa2e, BIT2, 0x0);
		...
		ODM_SetBBReg(pDM_Odm, 0x850, BIT31, 0x0);
		ODM_SetBBReg(pDM_Odm, 0x850, BIT23, 0x0);
	} else {
		/* 2R */

		/* Enable MRC for CCA */
		ODM_SetBBReg(pDM_Odm, 0xa2e, BIT6, 0x1);

		/* Enable MRC for barker */
		ODM_SetBBReg(pDM_Odm, 0xa2e, BIT2, 0x1);
		...
		ODM_SetBBReg(pDM_Odm, 0x850, BIT31, 0x1);
