	...
	)
{
	...
	ODM_SetBBReg(pDM_Odm, 0x14c0 , BIT7, 0); /*Disanable Tx MU-MIMO until sounding done*/
	...
	)
{
	...
	if ((pBeamformingInfo->beamformer_mu_cnt > 0) && (BFerIdx < BEAMFORMER_ENTRY_NUM)) {
		...
		ODM_SetBBReg(pDM_Odm, 0x14c0, 0x3F, 3);
		/*select MU STA1 table*/
		ODM_SetBBReg(pDM_Odm, 0x14c0, BIT10 | BIT9 | BIT8, 0);
		ODM_SetBBReg(pDM_Odm, 0x14c4, bMaskDWord, 0); /*Reset gid_valid table*/
		ODM_SetBBReg(pDM_Odm, 0x14c8, bMaskDWord, 0); /*Reset user position table*/
		ODM_SetBBReg(pDM_Odm, 0x14cc, bMaskDWord, 0); /*Reset user position table*/
		/*select MU STA2 table*/
		ODM_SetBBReg(pDM_Odm, 0x14c0, BIT10 | BIT9 | BIT8, 1);
		ODM_SetBBReg(pDM_Odm, 0x14c4, bMaskDWord, 0); /*Reset gid_valid table*/
		ODM_SetBBReg(pDM_Odm, 0x14c8, bMaskDWord, 0); /*Reset user position table*/
		ODM_SetBBReg(pDM_Odm, 0x14cc, bMaskDWord, 0); /*Reset user position table*/
	...
	if ((pBeamformingInfo->beamformee_mu_cnt > 0) && (BFeeIdx < BEAMFORMEE_ENTRY_NUM)) {
		...
		/*select MU STA table*/
		ODM_SetBBReg(pDM_Odm, 0x14c0 , BIT10 | BIT9 | BIT8, pBeamformeeEntry->mu_reg_index << 8);
		/*set validity of MU STAs*/
		ODM_SetBBReg(pDM_Odm, 0x14c0 , 0x3F, pBeamformingInfo->beamformee_mu_reg_maping);
	...
	)
{
	...
	if (pBeamformerEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		...
		if (pBeamformerEntry->is_mu_ap == 0) { /*SU BFfer */
			if (pBeamformerEntry->su_reg_index == 0) {	
			...
			/*set validity of MU STA0 and MU STA1*/
			ODM_SetBBReg(pDM_Odm, 0x14c0, 0x3F, 0);
			/*select MU STA1 table*/
			ODM_SetBBReg(pDM_Odm, 0x14c0, BIT10 | BIT9 | BIT8, 0);
			ODM_SetBBReg(pDM_Odm, 0x14c4, bMaskDWord, 0); /*Reset gid_valid table*/
			ODM_SetBBReg(pDM_Odm, 0x14c8, bMaskDWord, 0); /*Reset user position table*/
	...
	if (pBeamformeeEntry->BeamformEntryCap == BEAMFORMING_CAP_NONE) {
		...
		if (pBeamformeeEntry->is_mu_sta == 0) { /*SU BFee*/
			if (pBeamformeeEntry->su_reg_index == 0) {	
				...
				ODM_Write2Byte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8814A+2, 
				ODM_Read2Byte(pDM_Odm, REG_ASSOCIATED_BFMEE_SEL_8814A+2) & 0x60);
			}
			...
			ODM_SetBBReg(pDM_Odm, 0x14c0, BIT(pBeamformeeEntry->mu_reg_index), 0);
			/*select MU STA1 table*/
			ODM_SetBBReg(pDM_Odm, 0x14c0, BIT10 | BIT9 | BIT8, pBeamformeeEntry->mu_reg_index);
			ODM_SetBBReg(pDM_Odm, 0x14c4, bMaskDWord, 0); /*Reset gid_valid table*/
			ODM_SetBBReg(pDM_Odm, 0x14c8, bMaskDWord, 0); /*Reset user position table*/
			ODM_SetBBReg(pDM_Odm, 0x14cc, bMaskDWord, 0); /*Reset user position table*/
	...
	)
{
	...
	if (is_mu_sounding == FALSE) {
		...
		for (idx = 0; idx < 6; idx++) {
			ODM_SetMACReg(pDM_Odm, 0x14C0, BIT10|BIT9|BIT8, idx); /*select MU STA table*/
			ODM_SetMACReg(pDM_Odm, 0x14C4, bMaskDWord, gid_valid[idx]); /*set MU STA gid valid table*/
		}
		ODM_SetBBReg(pDM_Odm, 0x14c0 , BIT7, 1); /*Enable TxMU PPDU*/
	}
}
