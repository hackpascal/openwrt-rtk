...
#if defined(CONFIG_RTL_8198C) && defined(CONFIG_RTL_MLD_SNOOPING)
...
static void  _rtl8198C_setASICMulticastv6PortStatus(void) {
	uint32 index;
	for (index=0; index<RTL8651_PORT_NUMBER+rtl8651_totalExtPortNum; index++) {
		rtl8651_setAsicMulticastPortInternal(index, (rtl865x_externalMulticastPortMask&(1<<index))?FALSE:TRUE);//to be checked
	}
}
...
int32 rtl8198C_initMulticastv6(void)//rtl8198c_mCastv6Config_t * mCastConfig6Ptr)
{
...
int32 rtl8198C_addMulticastv6ExternalPort(uint32 extPort)
{
	...
	_rtl8198C_setASICMulticastv6PortStatus();//to be checked
	return SUCCESS;
}
...
int rtl8198C_setMCastv6HashMethod(unsigned int hashMethod)//to be checked
{
	uint32	  oldHashMethod = 0;
